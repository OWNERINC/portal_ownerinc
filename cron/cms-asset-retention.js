...
async function reserveCmsAssets(client, days) {
  try {
    ...
    const { rows } = await client.query(`
      SELECT a.id, a.storage_key
       ...
       WHERE a.created_at < NOW() - ($1::integer * INTERVAL '1 day')
         AND (a.deleting_at IS NULL OR a.deleting_at < NOW() - INTERVAL '1 hour')
         AND NOT EXISTS (
           SELECT 1
    ...
    for (const asset of reserved) {
      try {
        ...
        if (error.code === 'ENOENT') {
          ...
          fileFailures += 1;
          await client.query('UPDATE cms_assets SET deleting_at = NULL WHERE id = $1', [asset.id]);
          continue;
        }
      }
...
module.exports = { cmsAssetRetentionDays, enforceCmsAssetRetention, reserveCmsAssets, CMS_ASSET_RETENTION_LOCK };
