...
| Editor e exportação de convites | Operacional | Possui dois módulos alternáveis, Convidado (`convite_owntime`) e Owner (`convite_owner`, baseado no Frame 2 de 1448 × 4234 do Figma), ambos com campos de conteúdo editáveis, formatação rica segura, imagem, histórico, CRUD e exportação pelo PDF do navegador. Os dois módulos usam o rodapé SVG exportado do Figma em preview, responsivo e impressão; o Owner mantém capa, ícones, fonte Raleway local e página longa próprios. `public/cards-pos.html`, `public/cards-pos/app.js`, `public/cards-pos/assets/` |
