...
router.put('/:id', authMiddleware, async (req, res, next) => {
  ...
  try {
    const pageAccessValue = req.body.page_access === undefined
      ? null
      : JSON.stringify(normalizeJobTitlePages(req.body.page_access));
    const row = await withAudit(pool, req, 'job_title.update', 'job_title', async (db) => {
      const { rows } = await db.query(
         `UPDATE job_titles SET name = $2, active = $3, page_access = COALESCE($4::jsonb, page_access), updated_at = NOW()
          WHERE id = $1 RETURNING *`,
         [req.params.id, req.body.name.trim(), req.body.active, pageAccessValue]
       );
      return rows[0];
