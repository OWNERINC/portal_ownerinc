...
router.get('/categories', authMiddleware, async (req, res, next) => {
  ...
  const viewAll = mayViewAll(req.user, 'manageBenefits', all);
  const where = viewAll ? '' : 'WHERE active = TRUE';
  try {
    if (!viewAll) {
      const { rows } = await pool.query(
        `SELECT id, category FROM benefits ${where} AND btrim(category) <> '' ORDER BY category, id`,
      );
      const visible = (await addPublishedBlocks(pool, rows, 'benefit')).filter(isPublicCmsRow);
...
router.get('/', authMiddleware, async (req, res, next) => {
  ...
  try {
    if (!viewAll) {
      const { rows } = await pool.query(
        `SELECT * FROM benefits ${where} ORDER BY "order", id`,
        filterValues,
      );
      const visible = (await addPublishedBlocks(pool, rows, 'benefit')).filter(isPublicCmsRow);
...
router.delete('/:id', authMiddleware, async (req, res, next) => {
  ...
  try {
    const row = await withAudit(pool, req, 'benefit.delete', 'benefit',
      db => deleteCmsSource(db, 'benefit', req.params.id), { targetId: req.params.id });
    if (!row) return res.status(404).json({ error: 'Benefit not found.', requestId: req.id });
