...
CREATE TABLE IF NOT EXISTS user_import_jobs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), created_by TEXT REFERENCES users(uid) ON DELETE SET NULL,
  status TEXT NOT NULL DEFAULT 'queued' CHECK (status IN ('queued', 'processing', 'completed')),
  total_count INTEGER NOT NULL CHECK (total_count BETWEEN 1 AND 500), ready_count INTEGER NOT NULL CHECK (ready_count BETWEEN 0 AND 500),
